#ifndef GAME_OF_LIFE_MPI_IMPLEMENTAION
#define GAME_OF_LIFE_MPI_IMPLEMENTAION
...
namespace MPI
{
   ...
   std::string MPISerialSolve(int argc, char* argv[])
   {
      int procID, nproc;
      ...
      std::string result = "Proc " + std::to_string(procID) + " -- solve : " + Serial::serialSolve();
   ...
   // Exercice 4 : Only with 2 workers
   std::string MPIParallelSolve(int argc, char* argv[])
   {
      MPI_Status status;
      // Initialize MPI
      MPI_CHECK(MPI_Init(&argc, &argv), "failed to init");
      ...
      for (int loop = 0; loop < numberOfLoops; ++loop)
      {
         ...
         else
         {
            ...
            {
               // 1.2
               MPI_Send(&(*dataArrayOld)[1][1], 1, MPI_INT, previousProcID, 0, MPI_COMM_WORLD);
               MPI_Send(&(*dataArrayOld)[1][width], 1, MPI_INT, previousProcID, 1, MPI_COMM_WORLD);

               MPI_Recv(&(*dataArrayOld)[0][0], 1, MPI_INT, previousProcID, 2, MPI_COMM_WORLD, &status);
               MPI_Recv(&(*dataArrayOld)[0][width + 1], 1, MPI_INT, previousProcID, 3, MPI_COMM_WORLD, &status);

               // 1.3
               MPI_Send(&((*dataArrayOld)[1][1]), width, MPI_INT, previousProcID, 4, MPI_COMM_WORLD);
               MPI_Send(&((*dataArrayOld)[endHeight][1]), width, MPI_INT, previousProcID, 5, MPI_COMM_WORLD);

               MPI_Recv(&((*dataArrayOld)[0][1]), width, MPI_INT, previousProcID, 6, MPI_COMM_WORLD, &status);
               MPI_Recv(&((*dataArrayOld)[endHeight + 1][1]), width, MPI_INT, previousProcID, 7, MPI_COMM_WORLD, &status);
            }
            else if (procID == (nproc - 1))
            {
               // 1.2
               MPI_Recv(&(*dataArrayOld)[height + 1][width + 1], 1, MPI_INT, nextProcID, 0, MPI_COMM_WORLD, &status);
               MPI_Recv(&(*dataArrayOld)[height + 1][0], 1, MPI_INT, nextProcID, 1, MPI_COMM_WORLD, &status);

               MPI_Send(&(*dataArrayOld)[height][width], 1, MPI_INT, nextProcID, 2, MPI_COMM_WORLD);
               MPI_Send(&(*dataArrayOld)[height][1], 1, MPI_INT, nextProcID, 3, MPI_COMM_WORLD);

               // 1.3
               MPI_Recv(&(*dataArrayOld)[startHeight][1], width, MPI_INT, previousProcID, 4, MPI_COMM_WORLD, &status);
               MPI_Recv(&(*dataArrayOld)[endHeight][1], width, MPI_INT, previousProcID, 5, MPI_COMM_WORLD, &status);

               MPI_Send(&(*dataArrayOld)[startHeight - 1][1], width, MPI_INT, previousProcID, 6, MPI_COMM_WORLD);
               MPI_Send(&(*dataArrayOld)[endHeight + 1][1], width, MPI_INT, previousProcID, 7, MPI_COMM_WORLD);
            }
         }
